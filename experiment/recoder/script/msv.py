...
def get_correct_patch(bugid: str) -> str:
    with open("data/correct_patch.csv", "r") as f:
...
def run(bugid: str):
    ...
    if not has_correct_patch:
        with open("log/not-found.csv", "a") as f:
            f.write(f"{bugid}\n")
        print(f'no correct patch for {bugid}')
        return
    with open("log/found.csv", "a") as f:
        f.write(f"{bugid}\n")
    ...
    # os.system(f"cp -r {recoder_path}/out/{bugid}-recoder-{old_id} {recoder_path}/out-{id}/{bugid}-recoder-{id}")
    # os.system(f"cp -r {recoder_path}/out/{bugid}-seapr-{old_id} {recoder_path}/out-{id}/{bugid}-seapr-{id}")
    outdir = f"{recoder_path}/out-{id}/{bugid}-recoder-{id}"
    cmd = ["python3", "../casino.py", "-o", outdir, "-t", "180000", "-w", f"{workdir}", "-p", recoder_path, '-m', 'recoder',
            '--use-pass-test', '--recoder-mode',  "--use-simulation-mode", sim_data, "-c", correct_patch, "--finish-correct-patch", '--', 'python3', f'{msv_path}/script/d4j_run_test.py', recoder_path]
    execute(bugid, cmd, "recoder", outdir)
    outdir = f"{recoder_path}/out-{id}/{bugid}-seapr-{id}"
    cmd = ["python3", "../casino.py", "-o", outdir, "-t", "180000", "-w", f"{workdir}", "-p", recoder_path, '-m', 'seapr',
        '--use-pass-test', '--recoder-mode', '--use-simulation-mode', sim_data, "-c", correct_patch,'--finish-correct-patch', '--', 'python3', f'{msv_path}/script/d4j_run_test.py', recoder_path]
    execute(bugid, cmd, "seapr", outdir)
    for i in range(10):
        outdir = f"{recoder_path}/out-{id}/{bugid}-guided-{id}-{i}"
        cmd = ["python3", "../casino.py", "-o", outdir, "-t", "180000", "-w", f"{workdir}", "-p", recoder_path, '-m', 'guided',
            '--use-pass-test', '--recoder-mode', '--use-simulation-mode', sim_data, "-c", correct_patch,'--finish-correct-patch', '--seed', str(SEEDS[i]), '--', 'python3', f'{msv_path}/script/d4j_run_test.py', recoder_path]
        execute(bugid, cmd, f"guided-{i}", outdir)
    with open("log/finished.csv", "a") as f:
        f.write(f"{bugid}\n")
