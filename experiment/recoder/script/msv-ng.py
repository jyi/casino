...
def run(bugid: str):
    ...
    cmd = ["python3", "../casino.py", "-o", outdir, "-t", "180000", "-w", f"{workdir}", "-p", recoder_path, '-m', 'recoder',
           '--use-pass-test', '--recoder-mode', "--use-simulation-mode", sim_data, "-c", correct_patch, "--finish-correct-patch", "--cycle-limit", "10000", '--', 'python3', f'{msv_path}/script/d4j_run_test.py', buggy_path]
    ...
    for i in range(20):
        ...
        cmd = ["python3", "../casino.py", "-o", outdir, "-t", "180000", "-w", f"{workdir}", "-p", recoder_path, '-m', 'guided',
               '--use-pass-test', '--recoder-mode', '--use-simulation-mode', sim_data, "-c", correct_patch, '--cycle-limit', '10000', '--not-use-guide', '--seed', str(SEEDS[i]), "--finish-correct-patch", '--', 'python3', f'{msv_path}/script/d4j_run_test.py', buggy_path]
