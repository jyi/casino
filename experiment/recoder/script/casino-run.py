...
def run(bugid: str):
    ...
    cmd = ["python3", f"{casino_path}/casino.py", "-o", outdir, "-t", "180000", "-w", f"{workdir}", "-p", recoder_path, '-m', 'recoder',
           '--use-pass-test', '--recoder-mode', "--use-simulation-mode", sim_data, "-c", correct_patch, "--cycle-limit", "10000", "--finish-correct-patch", '--', 'python3', f'{casino_path}/script/d4j_run_test.py', buggy_path]
