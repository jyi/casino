...
def run(bugid: str):
    if bugid in ["Closure-109", "Math-2"]:
        return
    ...
    for i in range(20):
        ...
        cmd = ["python3", "../casino.py", "-o", outdir, "-t", "180000", "-w", f"{workdir}", "-p", recoder_path, '-m', 'guided',
               '--use-pass-test', '--recoder-mode', '--use-simulation-mode', sim_data, "-c", correct_patch, '--cycle-limit', '10000', '--seed', str(SEEDS[i]), '--', 'python3', f'{msv_path}/script/d4j_run_test.py', buggy_path]
