...
id = ""
old_id = ""
alpha_repair_path = "/root/project/alpha-repair"
casino_path = "/root/project/casino"

def get_correct_patch(bugid: str) -> str:
    with open(os.path.join(alpha_repair_path, "data", "correct_patch.csv"), "r") as f:
...
def run(bugid: str):
    ...
    with open("log/found-{id}.csv", "a") as f:
        f.write(f"{bugid}\n")
    workdir = os.path.join(alpha_repair_path, "d4j", bugid)
    sim_data = os.path.join(alpha_repair_path, "sim", bugid, f"{bugid}-sim.json")
    os.makedirs(os.path.dirname(sim_data), exist_ok=True)
    buggy_path = os.path.join(alpha_repair_path, f"buggy-{id}")
    os.makedirs(buggy_path, exist_ok=True)
    outdir = f"{alpha_repair_path}/out-{id}/{bugid}-recoder-{id}"
    cmd = ["python3", f"{casino_path}/casino.py", "-o", outdir, "-t", "180000", "-w", f"{workdir}", "-p", alpha_repair_path, '-m', 'recoder',
           '--use-pass-test', '--recoder-mode', "--use-simulation-mode", sim_data, "-c", correct_patch, "--cycle-limit", "10000", '--', 'python3', f'{casino_path}/script/d4j_run_test.py', buggy_path]
    execute(bugid, cmd, "recoder", outdir)
    outdir = f"{alpha_repair_path}/out-{id}/{bugid}-seapr-{id}"
    cmd = ["python3", f"{casino_path}/casino.py", "-o", outdir, "-t", "180000", "-w", f"{workdir}", "-p", alpha_repair_path, '-m', 'seapr',
           '--use-pass-test', '--recoder-mode', '--use-simulation-mode', sim_data, "-c", correct_patch, '--cycle-limit', '10000', '--ignore-compile-error', '--', 'python3', f'{casino_path}/script/d4j_run_test.py', buggy_path]
    execute(bugid, cmd, "seapr", outdir)
    for i in range(20):
        outdir = f"{alpha_repair_path}/out-{id}/{bugid}-guided-{id}-{i}"
        cmd = ["python3", f"{casino_path}/casino.py", "-o", outdir, "-t", "180000", "-w", f"{workdir}", "-p", alpha_repair_path, '-m', 'guided',
               '--use-pass-test', '--recoder-mode', '--use-simulation-mode', sim_data, "-c", correct_patch, '--cycle-limit', '10000', '--seed', str(SEEDS[i]), '--', 'python3', f'{casino_path}/script/d4j_run_test.py', buggy_path]
...
if sys.argv[1] == "gen":
    ...
    if id == "not-found":
        ...
        id = sys.argv[3]
        old_id = sys.argv[4]
    os.makedirs(alpha_repair_path + "/out-" + id, exist_ok=True)
    print("Setup casino!")
    # 53
