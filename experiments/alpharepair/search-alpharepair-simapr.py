...
def run(project,seed):
    ...
    result=subprocess.run(['python3',f'{new_cur_dir}/SimAPR/simapr.py','-o',f'result/{project}-simapr','-m','guided','--seed',f'{seed}','--use-exp-alpha',
                '--recoder-mode','-w',f'{new_cur_dir}/alpha-repair/d4j/{project}','-t','180000','--use-pass-test','--use-simulation-mode',f'result/{project}-cache.json',
                '-T','18000', '--','python3',
                f'{new_cur_dir}/SimAPR/script/d4j_run_test.py',f'{new_cur_dir}/alpha-repair/buggy'],stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
...
if __name__ == '__main__':
    args=sys.argv
    if len(args)!=3:
        print('Usage: python3 search-alpharepair-simapr.py <project> <seed>')
        sys.exit(1)
