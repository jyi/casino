...
def run(project):
    ...
    result=subprocess.run(['python3',f'{new_cur_dir}/SimAPR/simapr.py','-o',f'result/{project}-seapr','-m','seapr','--ignore-compile-error','--use-pattern',
                '--tbar-mode','-w',f'{new_cur_dir}/TBar/d4j/{project}','-t','180000','--use-pass-test','--use-simulation-mode',f'result/{project}-cache.json','-T','18000',
                '--','python3',
...
if __name__ == '__main__':
    args=sys.argv
    if len(args)!=2:
        print('Usage: python3 search-tbar-seapr.py <project>')
        sys.exit(1)
